...
BASE_URL = (
    "https://prod-rel-ffc.oobesaas.adobe.com/adobe-ffc-external/aamee/v2/products/all"
)
...
def feed_url(channels, platforms):
    ...
    params = [("payload", "true"), ("productType", "Desktop"), ("_type", "json")]
    for ch in channels:
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "dump":
        ...
        for sapcode, productVersions in products.items():
            ...
            for product in productVersions:
                base_version = product["platforms"]["platform"][0]["languageSet"][
                    0
                ].get("baseVersion")
                if not base_version:
                    base_version = "N/A"
                ...
                print(
                    (
                        "\t{0: <60}\tBaseVersion: {1: <14}\tVersion: {2: <14}".format(
                            name, base_version, product["version"]
                        )
                    )
                )
            print("")
